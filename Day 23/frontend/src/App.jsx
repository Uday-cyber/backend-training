...
const LoginForm = ({ onSuccess, onShowRegister }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!data?.token) return setError("Wrong credentials");
    ...
    <CenterCard
      ...
      footer={
        <div className="mt-6 flex items-center justify-between">
          <SubtleButton onClick={onShowRegister}>
            Register new user
          </SubtleButton>
          <button
            onClick={handleLogin}
...
const TaskRow = ({ task, onEdit, onDelete }) => (
  <tr className="border-b last:border-none">
    ...
    <td className="px-4 py-3 text-right">
      <button
        onClick={() => onEdit(task)}
        className="mr-2 border px-3 py-1.5 rounded-lg hover:bg-slate-50"
      >
        Update
      </button>
      <button
...
const TasksScreen = ({ onLogout }) => {
  ...
  const handleCreate = async ({ title, description, status, file }) => {
    try {
      ...
      socket.emit("taskCreated", { title, description, status });
      setOpenAdd(false);
      fetchTasks();
    } catch {
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-6xl mx-auto p-4">
        ...
        <div className="flex flex-col md:flex-row justify-between mb-4 gap-3">
          ...
          <div className="text-sm text-slate-600 mt-1">Total: {total}</div>
        </div>
